...
case class TicketView(id: Int, ticket_number: String, school_bucket:String, school_number:String, school_name:String)

object TicketView {

  implicit val ticketFormat = Json.format[TicketView]

  implicit val ticketReader = Json.reads[TicketView]

  implicit val ticketWriter = Json.writes[TicketView]

  val listWrites: Writes[List[TicketView]] = Writes.traversableWrites[TicketView](ticketWriter)
}
...
object Ticket {
  ...
  val ticketWrites: Writes[Ticket] =
    (
        (__ \ "id").write[Int] ~
  ...
  val listWritesWithNewFields: Writes[List[Ticket]] = Writes.traversableWrites[Ticket](ticketWrites)
}
